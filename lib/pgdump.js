...
function spawnPgDump(config) {
    ...
    if ("EXCLUDE_TABLES" in config) {
        config.EXCLUDE_TABLES.forEach(function (table) {
            excludeTables.push(`--exclude-table-data=${table}`);
        })
    }
    console.log("EXCLUDE_TABLES: ", excludeTables)
    return spawn(pgDumpPath, excludeTables.concat(['-Fc']), {
        env
    })
}
